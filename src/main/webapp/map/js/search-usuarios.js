...
$(document).ready(function () {
	...
	$('#btn-qtd-status').on('click', function () {

		var status = $('#quantidade-status').val();

		$.ajax({
			url: 'http://localhost:8282/sniet_api/servlet/usuarios/status/' + status,
			type: 'GET',
			crossDomain: true,
			contentType: 'text/plain; charset=utf-8',
			dataType: 'text',
	...
	function criaTabelaUsuarios(users) {
		...
		for (var user in users) {
			trUsuario +=
				"<!-- usuarios -->" +
				"<tr>" +
				"<td id='id-user'>" + users[user].idUsuario + "</td>" +
				...
				"<td> <input type='button' data-id=" + users[user].id +
				" class='btn-delete' value='Apagar'> </td>" +

				"<td> <input type='button' data-id=" + users[user].id +
				" class='btn-delete-id' value='Apagar ID'> </td>" +
				"</tr>" +
				"<!-- usuarios -->";
		}
		...
		$('#tbl-users-body').on('click', '.btn-delete', function () {
			deleteUsuario(users[$(this).index()]);
		})


		$('#tbl-users-body').on('click', '.btn-delete-id', function () {
			deleteUsuarioID(users[$(this).index()].id);
		})
	...
	function deleteUsuarioID(id_user) {
